<template>
   <div v-if="searchResults" class="flex justify-center border-darkKhaki border-b-2 first:border-t-2 ">
    <div class="capitalize w-7/12 tablet:w-9/12 border-darkKhaki  border-r-2 p-2 pl-3 flex items-center">
      {{ item.item }}
    </div>
    <div class="capitalize w-3/12 tablet:w-2/12 border-darkKhaki  border-r-2 p-1 text-center">
      <input
        min="1"
        type="number"
        v-model="amount"
    ...
    <button
      ...
      class="group w-2/12 tablet:w-1/12 flex justify-center items-center border-darkKhaki"
    >
      <FaPlus class="fill-darkKhaki group-hover:fill-maroon w-5 h-5" />
    </button>
  </div>
  <div v-else class="flex">
    <div class="capitalize w-7/12 tablet:w-9/12 border-darkKhaki border-b-2 border-r-2 p-2 pl-3 flex items-center">
      {{ item.item }}
    </div>
    <div class="capitalize w-3/12 tablet:w-2/12 border-darkKhaki border-b-2 border-r-2 p-1 text-center">
      <input
        min="1"
        type="number"
    ...
    <button
      ...
      class="group w-2/12 tablet:w-1/12 flex justify-center items-center border-b-2 border-darkKhaki"
    >
